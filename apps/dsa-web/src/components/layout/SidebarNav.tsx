...
import { BarChart3, BriefcaseBusiness, FlaskConical, Home, LogOut, MessageSquareQuote, Settings2 } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'quant', label: '量化研究', to: '/quant', icon: FlaskConical },
      ...
      <nav className="flex flex-1 flex-col gap-1.5" aria-label="主导航">
        {NAV_ITEMS.map(({ key, label, to, icon: Icon, exact, badge }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                'group relative -ml-1 mr-3 flex items-center gap-3 overflow-hidden rounded-xl border text-sm transition-all',
              ...
              <>
                {isActive && (
                  <motion.div
                    ...
                    className="absolute top-0 bottom-0 left-0 w-[var(--nav-indicator-width)] rounded-l-xl bg-[var(--nav-indicator-bg)] shadow-[0_0_10px_var(--nav-indicator-shadow)]"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
