...
- [新功能] 新增 Firstrade 只读 Broker Connector：`src/brokers/firstrade/`（lazy `from firstrade import account`，永不 import `firstrade.order`） + `src/services/firstrade_sync_service.py` + `src/repositories/broker_snapshot_repo.py` + `api/v1/endpoints/broker.py`（9 个端点：status / login / verify / sync / accounts / positions / orders / transactions / snapshot），同步账户 / 余额 / 持仓 / 订单 / 交易历史快照到 `broker_sync_runs` + `broker_snapshots` 两张表。**严格只读** — 不实现下单 / 撤单 / 期权交易；账号经 `mask_account_number` + `hash_account_number` 脱敏成 `Firstrade ****1234` + 16 位 SHA256；payload 三层 redaction（client → repo → API endpoint 数字串 regex 兜底）；MFA 两步流支持且 Cloud Run 实例回收时返回 HTTP 409 `session_lost`。
- [新功能] 新增 Agent 券商持仓快照工具 `get_live_broker_portfolio_snapshot`：仅在 `BROKER_FIRSTRADE_ENABLED=true` 时由 `factory.get_tool_registry()` 条件注册；只读本地 SQLite 快照，**永远不会登录 Firstrade、不会同步、不会下单**；输出按 `BROKER_FIRSTRADE_LLM_DATA_SCOPE` 投影（`positions_only` / `positions_and_balances` / `full`），始终带 `as_of_iso` + `age_seconds`，超过 `max_age_seconds` 默认 1h 时返回 `status="stale"` + warning 并仍提供数据。
- [改进] Portfolio WebUI 页面顶部新增 Firstrade 只读同步面板（`apps/dsa-web/src/components/portfolio/FirstradeSyncPanel.tsx`），状态机 `disabled / not_installed / logged_out / mfa_required / logged_in / syncing`，复用现有 `Card` / `InlineAlert` / `ApiErrorAlert` / `Input(type=password, iconType=key)`；显式标注「研究专用 · 永远不会下单或撤单」；浏览器永远不存储 username / password / MFA / cookie / token。
- [文档] 新增 `docs/firstrade-integration.md` 描述功能定位、风险（非官方 reverse-engineered API）、安装、15 个环境变量、MFA 流程、API / Agent / WebUI 用法、安全边界、故障排查、回滚方式。
- [测试] 新增 `tests/test_firstrade_client.py`（20 例）/ `tests/test_firstrade_sync_service.py`（12 例）/ `tests/test_broker_api.py`（11 例，含 5 个端点的**参数化漏值测试**：注入 username / password / pin / mfa_secret / ftat / sid / cookie / Authorization / 完整账号到 mock 响应，验证 API 输出的敏感 value 被 `***REDACTED***` 替换且不出现于响应字符串）/ `tests/test_agent_broker_tools.py`（13 例，含 scope 投影 / stale flag / 工具不调 firstrade client / `BROKER_FIRSTRADE_ENABLED=false` 时不注册）。
- [改进] `src/config.py` 增加 15 个 `BROKER_FIRSTRADE_*` 字段（含 `BROKER_ACCOUNT_HASH_SALT`），`_normalize_broker_llm_scope` helper 对非法 scope 回退到 `positions_and_balances`，`BROKER_FIRSTRADE_SYNC_INTERVAL_SECONDS` clamp `[30, 3600]`，启动期 fail-fast 拒绝 `enabled=true` 但 salt 为空的配置（防止跨部署可关联弱哈希）。
